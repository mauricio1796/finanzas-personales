...
export const ModalCategoria: React.FC<Props> = ({ visible, editing, existingCount, onClose, onSave }) => {
  ...
  const handleSave = () => {
    ...
    const cat: Category = {
      ...
      name: nombre.trim(),
      icon: emoji,
      color: editing?.color ?? COLORES[existingCount % COLORES.length],
      budget: monto,
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={s.kavWrap} pointerEvents="box-none">
        <Animated.View style={[s.sheet, { transform: [{ translateY: slideAnim }] }]}>
          ...
          <ScrollView style={s.body} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <TextInput style={[s.input, Platform.OS === 'web' && ({ outline: 'none' } as any)]} placeholder="Ej: Gimnasio, Netflix..." placeholderTextColor={colors.textTertiary} value={nombre} onChangeText={t => { setNombre(t); setError(""); }} maxLength={30} />
            <Text style={s.fieldLabel}>Presupuesto mensual (COP)</Text>
            <View style={s.amountRow}>
              <Text style={s.currencySign}>$</Text>
              <TextInput style={[s.input, s.amountInput, Platform.OS === 'web' && ({ outline: 'none' } as any)]} keyboardType={Platform.OS === 'web' ? 'default' : 'numeric'} placeholder="0" placeholderTextColor={colors.textTertiary} value={presupuesto} onChangeText={t => setPresupuesto(formatInput(t))} />
            </View>
            ...
            <TextInput style={[s.input, Platform.OS === 'web' && ({ outline: 'none' } as any)]} keyboardType={Platform.OS === 'web' ? 'default' : 'numeric'} placeholder="Sin fecha fija" placeholderTextColor={colors.textTertiary} value={diaPago} onChangeText={t => { const v = t.replace(/[^0-9]/g,""); setDiaPago(v.length > 2 ? v.slice(0,2) : v); }} maxLength={2} />
...
const makeStyles = (colors: AppColors) => StyleSheet.create({
  ...
  hdr:            { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 20, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: colors.border },
  title:          { fontSize: 17, fontWeight: "700", color: colors.textPrimary },
  cancel:         { fontSize: 15, color: colors.primary, fontWeight: "600" },
  body:           { paddingHorizontal: 20, paddingTop: 16 },
  fieldLabel:     { fontSize: 12, fontWeight: "700", color: colors.textTertiary, letterSpacing: 0.8, marginBottom: 6, marginTop: 14 },
  input:          { backgroundColor: colors.background, borderWidth: 1, borderColor: colors.border, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, color: colors.textPrimary },
  amountRow:      { flexDirection: "row", alignItems: "center", gap: 8 },
  currencySign:   { fontSize: 18, fontWeight: "700", color: colors.textPrimary },
  amountInput:    { flex: 1 },
  tipoRow:        { flexDirection: "row", gap: 10 },
  tipoBtn:        { flex: 1, borderWidth: 1, borderColor: colors.border, borderRadius: THEME.radius.md, padding: 12, alignItems: "center", gap: 4 },
  tipoBtnActive:  { borderColor: colors.primary, backgroundColor: colors.primaryLight },
  tipoEmoji:      { fontSize: 20 },
  tipoText:       { fontSize: 14, fontWeight: "700", color: colors.textSecondary },
  tipoTextActive: { color: colors.primary },
  tipoSub:        { fontSize: 10, color: colors.textTertiary, textAlign: "center" },
  tipoSubActive:  { color: colors.primary },
  emojiScroll:    { marginBottom: 4 },
  emojiBtn:       { width: 44, height: 44, borderRadius: 10, alignItems: "center", justifyContent: "center", marginRight: 8, backgroundColor: colors.cardSecondary },
  ...
  error:          { color: colors.expense, fontSize: 13, fontWeight: "600", marginBottom: 8, marginTop: 4 },
});
